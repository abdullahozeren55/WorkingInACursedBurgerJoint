...
public class CustomerManager : MonoBehaviour
{
    ...
    public void UpdateMonitorWithGroupOrders()
    {
        ...
        // 2. SÝPARÝÞLERÝ LÝSTELE (Toplama Yok!)
        List<OrderData> groupOrders = new List<OrderData>();

        foreach (var customer in customers)
        {
            // Adamýn sipariþi var mý?
            // Not: GetCurrentOrder() metodunu CustomerController'a eklemiþtik, onu kullanýyoruz.
            OrderData individualOrder = customer.GetCurrentOrder();

            if (individualOrder != null)
            {
                // Direkt listeye ekle. 
                // Ahmet'in sipariþi [0], Mehmet'in sipariþi [1] olacak.
                groupOrders.Add(individualOrder);
            }
        }

        // 3. Listeyi Monitöre Gönder
